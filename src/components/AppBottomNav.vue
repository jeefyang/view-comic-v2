...
<template>
    <n-layout-footer bordered position="absolute" style="height: 60px; padding: 0; display: flex">
        <div v-for="item in navItems" :key="item.path" class="nav-item" :class="{ active: isActive(item.path) }">
            <div class="flex flex-col items-center gap-1 p-2" @click="$router.push(item.path)" :style="{ color: isActive(item.path) ? themeVars.primaryColor : undefined }">
...
const navItems = [
    ...
    { label: "书架", path: "/shelf", icon: BookOutline },
